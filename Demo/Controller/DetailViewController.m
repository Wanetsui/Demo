...
@property (nonatomic, strong) ZFKSControlView *controlView;
...
- (void)viewDidLoad
{
	...
	@weakify(self)
	if (!self.player) {
		self.player = [PlayerController playrWithContainerView:[UIView new]];
		[self requestData];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
	ZFKSCell *cell = [tableView dequeueReusableCellWithIdentifier:kIdentifier];
...
- (UITableView *)tableView {
	if (!_tableView) {
		...
		[_tableView registerClass:[ZFKSCell class] forCellReuseIdentifier:kIdentifier];
...
- (ZFKSControlView *)controlView {
	if (!_controlView) {
		_controlView = [ZFKSControlView new];
	}
	return _controlView;
}
